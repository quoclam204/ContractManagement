...
- [ ] Thiết kế entity: `CONTRACT_TYPES`, `TEMPLATES` (Gắn data annotation đặt vào Schema `Contract`).
- [ ] Viết Use Cases CRUD cho Loại hợp đồng và Mẫu hợp đồng (hỗ trợ lưu TemplateVersion).
...
- [ ] Thiết kế entity: `CONTRACTS` (Schema `Contract`).
- [ ] Viết Use Cases: Tạo hợp đồng (Draft), Cập nhật thông tin, Lấy chi tiết hợp đồng.
...
## Giao diện & Testing
- [ ] Phát triển UI trong route/thư mục được giao (VD: `/features/contracts`). Không sửa file Router gốc (Người 1 đã lo).
- [ ] Gọi API lấy danh sách Template và luồng tạo hợp đồng nháp.
